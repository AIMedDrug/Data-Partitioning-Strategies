...
output_dir = '/home/data1/BGM/fenziduiqi/All_X-_UNIPORT_4179_BiLSTM_result'
...
for hidden_dim in hidden_dim_list:
    for dropout in dropout_list:
        for num_layers in num_layers_list:
            ...
            # 保存预测图
            figname = f'BiLSTM_prediction_hidden{hidden_dim}_drop{dropout}_layers{num_layers}.png'
            plt.savefig(os.path.join(output_dir, figname), dpi=300)
            plt.close()
            
            # 保存预测结果
            prediction_file = f'BiLSTM_prediction_hidden{hidden_dim}_drop{dropout}_layers{num_layers}.csv'
...
results_df.to_csv(os.path.join(output_dir, 'BiLSTM_UNIPORT_results.csv'), index=False)