...
output_dir = '/home/data1/BGM/fenziduiqi/All_X-_UNIPORT_4179_RF_result'
...
for n_estimators in n_estimators_list:
    for max_depth in max_depth_list:
        for min_samples_split in min_samples_split_list:
            ...
            # 保存预测图
            figname = f'RF_prediction_estimators{n_estimators}_depth{max_depth}_split{min_samples_split}.png'
            output_path = os.path.join(output_dir, figname)
            plt.savefig(output_path, dpi=900)
            plt.close()
            
            # 保存预测结果
            prediction_file = f'RF_prediction_estimators{n_estimators}_depth{max_depth}_split{min_samples_split}.csv'
            ...
            # 记录皮尔逊结果
            pearson_results.append((n_estimators, max_depth, min_samples_split, pearson_corr))
...
best_depth = None
best_split = None
for estimators, depth, split, corr in pearson_results:
    if corr > max_corr:
        max_corr = corr
...
print(f"Best Parameters: n_estimators={best_estimators}, max_depth={best_depth}, min_samples_split={best_split}")

# 保存皮尔逊结果
results_df = pd.DataFrame(
    pearson_results,
    columns=['N Estimators', 'Max Depth', 'Min Samples Split', 'Pearson Corr']
)
results_df.to_csv(os.path.join(output_dir, 'RF_UNIPORT_results.csv'), index=False)