...
output_dir = '/home/data1/BGM/fenziduiqi/All_X-_GRU_4179_best_result'
...
for idx in range(30):
    ...
    X_train = np.concatenate((X1_train - X2_train, ecfp_train), axis=1)
    ...
    X_val = np.concatenate((X1_val - X2_val, ecfp_val), axis=1)
    ...
    X_test = np.concatenate((X1_test - X2_test, ecfp_test), axis=1)
    ...
    print(f"MSE: {mse:.4f}, MAE: {mae:.4f}, RMSE: {rmse:.4f}, Pearson: {pearson_corr:.4f}, Spearman: {spearman_corr:.4f}")
    ...
    # 保存预测图
    figname = f'GRU_prediction_random{idx}_hidden{hidden_dim}_drop{dropout}_layers{num_layers}.png'
    output_path = os.path.join(output_dir, figname)
    plt.savefig(output_path, dpi=900)
    plt.close()
    
    # 保存预测结果
    prediction_file = f'GRU_prediction_random{idx}_hidden{hidden_dim}_drop{dropout}_layers{num_layers}.csv'
...
results_df.to_csv(os.path.join(output_dir, 'GRU_best_results.csv'), index=False)