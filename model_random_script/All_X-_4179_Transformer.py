...
output_dir = '/home/data1/BGM/fenziduiqi/All_X-_Transformer_4179_result'
        ...
        for dropout in dropout_list:
            for num_layers in num_layers_list:
                for nhead in nhead_list:
                    ...
                    # 保存预测图
                    figname = f'Transformer_prediction_random{idx}_hidden{hidden_dim}_drop{dropout}_layers{num_layers}_nhead{nhead}.png'
                    plt.savefig(os.path.join(output_dir, figname), dpi=300)
                    plt.close()
                    
                    # 保存预测结果
                    prediction_file = f'Transformer_prediction_random{idx}_hidden{hidden_dim}_drop{dropout}_layers{num_layers}_nhead{nhead}.csv'
                    ...
                    pearson_results.append((random_seed, hidden_dim, dropout, num_layers, nhead, pearson_corr))
...
best_nhead = None
for seed, hidden_dim, dropout, num_layers, nhead, corr in pearson_results:
    if corr > max_corr:
        max_corr = corr
        best_seed = seed
...
print(f"Best Parameters: random_seed={best_seed}, hidden_dim={best_hidden_dim}, dropout={best_dropout}, num_layers={best_num_layers}, nhead={best_nhead}")

# 保存皮尔逊结果
results_df = pd.DataFrame(
    pearson_results,
    columns=['Random Seed', 'Hidden Dim', 'Dropout', 'Num Layers', 'Nhead', 'Pearson Corr']
)
