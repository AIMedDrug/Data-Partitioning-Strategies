...
output_dir = '/home/data1/BGM/fenziduiqi/All_X-_4179_BiLSTM_Q13315_random_result'
...
for ratio in ratios:
    for idx in range(30):
        ...
        # 数据集划分
        test_df = df[df['uniprot_ids'] == 'Q13315']
        train_df = df[df['uniprot_ids'] != 'Q13315']
        ...
        plt.title('True vs Predicted ΔΔG - BiLSTM', fontsize=46)
        plt.xlim(-5.5, 5)
        ...
        # 保存预测图
        figname = f'BiLSTM_prediction_ratio{ratio}_random{idx}.png'
        output_path = os.path.join(output_dir, figname)
        plt.savefig(output_path, dpi=900)
        plt.close()

        # 保存预测结果
        prediction_file = f'BiLSTM_prediction_ratio{ratio}_random{idx}.csv'
...
results_df.to_csv(os.path.join(output_dir, 'BiLSTM_pearson_ratio_results.csv'), index=False)