...
output_dir = '/home/data1/BGM/fenziduiqi/All_X-_UNIPORT_4179_Transformer_result'
    ...
    for dropout in dropout_list:
        for num_layers in num_layers_list:
            for nhead in nhead_list:
                ...
                # 检查 input_dim 是否能被 nhead 整 ACCEPTED除
                input_dim = X_train.shape[1]
                if input_dim % nhead != 0:
                    print(f"Skipping nhead={nhead} as input_dim={input_dim} is not divisible by nhead")
                    continue
                ...
                print(f"Parameters: hidden_dim={hidden_dim}, dropout={dropout}, num_layers={num_layers}, nhead={nhead}")
                ...
                plt.title('True vs Predicted DDG - Transformer', fontsize=8)
                ...
                # 保存预测图
                figname = f'Transformer_prediction_hidden{hidden_dim}_drop{dropout}_layers{num_layers}_nhead{nhead}.png'
                plt.savefig(os.path.join(output_dir, figname), dpi=300)
                plt.close()
                
                # 保存预测结果
                prediction_file = f'Transformer_prediction_hidden{hidden_dim}_drop{dropout}_layers{num_layers}_nhead{nhead}.csv'
                ...
                pearson_results.append((hidden_dim, dropout, num_layers, nhead, pearson_corr))
...
best_nhead = None
for hidden_dim, dropout, num_layers, nhead, corr in pearson_results:
    if corr > max_corr:
        max_corr = corr
...
print(f"Best Parameters: hidden_dim={best_hidden_dim}, dropout={best_dropout}, num_layers={best_num_layers}, nhead={best_nhead}")

# 保存皮尔逊结果
results_df = pd.DataFrame(
    pearson_results,
    columns=['Hidden Dim', 'Dropout', 'Num Layers', 'Nhead', 'Pearson Corr']
)
results_df.to_csv(os.path.join(output_dir, 'Transformer_UNIPORT_results.csv'), index=False)