...
output_dir = '/home/data1/BGM/fenziduiqi/All_X-_DNN_4179_best_result'
...
for idx in range(30):
    ...
    plt.title('True vs Predicted ΔΔG - DNN', fontsize=46)
    plt.xlim(-5.5, 5)
    ...
    # 保存预测图
    figname = f'DNN_prediction_random{idx}_hidden{hidden_dim}_drop{dropout}_layers{num_layers}.png'
    output_path = os.path.join(output_dir, figname)
    plt.savefig(output_path, dpi=900)
    plt.close()
    
    # 保存预测结果
    prediction_file = f'DNN_prediction_random{idx}_hidden{hidden_dim}_drop{dropout}_layers{num_layers}.csv'
...
results_df.to_csv(os.path.join(output_dir, 'DNN_best_results.csv'), index=False)