...
output_dir = '/home/data1/BGM/fenziduiqi/All_X-_4179_DNN_Q13315_UNIPORT_result'
...
class DNNRegressor(nn.Module):
    def __init__(self, input_dim, hidden_dim=16, num_layers=2, dropout=0.1):
...
for ratio in ratios:
    for idx in range(30):
        ...
        # 保存预测图
        figname = f'DNN_prediction_ratio{ratio}_random{idx}.png'
        output_path = os.path.join(output_dir, figname)
        plt.savefig(output_path, dpi=900)
        plt.close()

        # 保存预测结果
        prediction_file = f'DNN_prediction_ratio{ratio}_random{idx}.csv'
...
results_df.to_csv(os.path.join(output_dir, 'DNN_pearson_ratio_results.csv'), index=False)