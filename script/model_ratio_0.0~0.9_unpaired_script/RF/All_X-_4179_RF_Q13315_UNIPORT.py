...
output_dir = '/home/data1/BGM/fenziduiqi/All_X-_4179_RF_Q13315_UNIPORT_result'
...
for ratio in ratios:
    for idx in range(30):
        ...
        print(f"Test set size (Q13315): {len(test_df)}")
        ...
        # 打印数据集大小
        print(f"\nRatio: {ratio*100}%")
        print(f"Anchor set 大小: {len(test_anchor)}")
        print(f"Query set 大小: {len(test_query)}")
        ...
        # 保存预测图
        figname = f'RF_prediction_ratio{ratio}_random{idx}.png'
        output_path = os.path.join(output_dir, figname)
        plt.savefig(output_path, dpi=900)
        plt.close()

        # 保存预测结果
        prediction_file = f'RF_prediction_ratio{ratio}_random{idx}.csv'
...
results_df.to_csv(os.path.join(output_dir, 'RF_pearson_ratio_results.csv'), index=False)