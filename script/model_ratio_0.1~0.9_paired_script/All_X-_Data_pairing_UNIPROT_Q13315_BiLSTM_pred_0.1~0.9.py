...
root_output_dir = '/home/bioinfor6/BGM/fenziduiqi/All_X-_Data_pairing_UNIPROT_Q13315_BiLSTM_pred_result_0.1~0.9'
...
class BiLSTMRegressor(nn.Module):
    def __init__(self, input_dim, hidden_dim=1024, num_layers=2, dropout=0.1):
...
for ratio in anchor_ratios:
    for idx in range(30):
        ...
        output_dir = os.path.join(root_output_dir, f'All_X-_Data_pairing_UNIPROT_Q13315_BiLSTM_pred_result_{ratio}_seed{random_number}')
        ...
        def prepare_data_for_bilstm(pairs_X, pairs_y):
            X = np.stack(pairs_X, axis=0)
            y = np.array(pairs_y)
            return X, y

        X_train, y_train = prepare_data_for_bilstm(tX, ty)
        X_test, y_test = prepare_data_for_bilstm(aqX, aqy)
        ...
        results_df.to_csv(os.path.join(output_dir, 'BiLSTM_prediction_results.csv'), index=False)
        ...
        avg_df.to_csv(os.path.join(output_dir, 'BiLSTM_prediction_averages.csv'), index=False)
        ...
        plt.title('Query True vs Predicted ΔΔG - BiLSTM', fontsize=46)
        plt.xlim(-5.5, 5)
        ...
        figname    = f'BiLSTM_query_true_vs_pred_hid{hidden_dim}_drop{dropout}_layers{num_layers}_query_anchor.png'
