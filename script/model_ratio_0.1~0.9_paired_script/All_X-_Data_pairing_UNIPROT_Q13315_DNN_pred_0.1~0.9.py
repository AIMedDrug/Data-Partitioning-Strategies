...
root_output_dir = '/home/data1/BGM/fenziduiqi/All_X-_Data_pairing_UNIPROT_Q13315_DNN_pred_result_0.1~0.9'
...
def make_pairs(df, x1_col, x2_col, ecfp_col, ddg_col, uniprot_col):
    ...
    for uniprot, group in grouped:
        ...
        for i, j in combinations(idxs, 2):
            ...
            X_pair = np.concatenate([x1, x2])  # DNN直接拼接
...
class DNNRegressor(nn.Module):
    def __init__(self, input_dim, hidden_dim=16, num_layers=1, dropout=0.1):
...
for ratio in anchor_ratios:
    for idx in range(30):
        ...
        output_dir = os.path.join(root_output_dir, f'All_X-_Data_pairing_UNIPROT_Q13315_DNN_pred_result_{ratio}_seed{random_number}')
        ...
        X_train = np.stack(tX, axis=0)
        y_train = np.array(ty)
        X_test = np.stack(aqX, axis=0)
        y_test = np.array(aqy)
        ...
        results_df.to_csv(os.path.join(output_dir, 'DNN_prediction_results.csv'), index=False)
        ...
        avg_df.to_csv(os.path.join(output_dir, 'DNN_prediction_averages.csv'), index=False)
        ...
        plt.title('Query True vs Predicted ΔΔG - DNN', fontsize=46)
        plt.xlim(-5.5, 5)
        ...
        figname    = f'DNN_query_true_vs_pred_hid{hidden_dim}_drop{dropout}_layers{num_layers}_query_anchor.png'
