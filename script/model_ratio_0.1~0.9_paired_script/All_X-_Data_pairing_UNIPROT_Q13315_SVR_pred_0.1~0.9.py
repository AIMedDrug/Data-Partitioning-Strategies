...
root_output_dir = '/home/bioinfor6/BGM/fenziduiqi/All_X-_Data_pairing_UNIPROT_Q13315_SVR_pred_result_0.1~0.9'
...
def make_pairs(df, x1_col, x2_col, ecfp_col, ddg_col, uniprot_col):
    ...
    for uniprot, group in grouped:
        ...
        for i, j in combinations(idxs, 2):
            ...
            X_pair = np.concatenate([x1, x2])  # 拼接成一维向量
...
def make_cross_pairs(df_anchor, df_query, x1_col, x2_col, ecfp_col, ddg_col, uniprot_col):
    ...
    for j, i in product(query_indices, anchor_indices):
        ...
        X_pair = np.concatenate([x_query, x_anchor])
...
def save_pairs_to_csv(pairs_X, pairs_y, pairs_info, output_path):
    X_flat = []
    for pair in pairs_X:
        X_flat.append(list(map(str, pair)))
    df_pairs = pd.DataFrame(X_flat)
    df_pairs['ddG_diff'] = list(map(str, pairs_y))
    info_df = pd.DataFrame(pairs_info, columns=['idx1', 'idx2', 'UNIPROT']).astype(str)
    df_out = pd.concat([df_pairs, info_df], axis=1)
...
for ratio in anchor_ratios:
    for idx in range(30):
        ...
        output_dir = os.path.join(root_output_dir, f'All_X-_Data_pairing_UNIPROT_Q13315_SVR_pred_result_{ratio}_seed{random_number}')
        ...
        def prepare_data_for_svr(pairs_X, pairs_y):
            X = np.stack(pairs_X, axis=0)
            X = X.reshape(X.shape[0], -1)  # [num_samples, features]
            y = np.array(pairs_y)
            return X, y

        X_train, y_train = prepare_data_for_svr(tX, ty)
        X_test, y_test = prepare_data_for_svr(aqX, aqy)
        ...
        avg_df.to_csv(os.path.join(output_dir, 'SVR_prediction_averages.csv'), index=False)
        ...
        plt.title('Query True vs Predicted ΔΔG - SVR', fontsize=46)
        plt.xlim(-5.5, 5)
        ...
        figname    = f'SVR_query_true_vs_pred_C{C}_eps{epsilon}_{kernel}_query_anchor.png'
